...
def main_menu
  ...
  if (1..4).include?(item)
    ...
    puts "Данного пункта меню не существует, повторите ввод"
    main_menu
  end
end
...
def routes_menu
  if $config[:stations].empty?
    puts "Создайте хотя бы две станции чтобы создавать маршруты"
    station_menu
  else
...
def station_menu_controller(item)
  ...
  when 2..last_station_index
    ...
    if (station_item == 1) || (station_item == 2)
      ...
      puts "Вы ввели неверный пункт меню, повторите ввода"
      station_menu_controller(item)
    end
  else
    puts "Произошла ошибка ввода пункта меню, вы были возвращены на предыдущий экран"
    station_menu
  end
end
...
def train_menu_controller(item, chosen_train_number)
  ...
  when 3
    if $config[:routes].empty?
      puts "У вас отсутствуют доступные маршруты, добавьте их"
      main_menu
    else
  ...
  when 5
    ...
    train_menu(chosen_train_number)
  when 6
    $config[:trains][chosen_train_number].move_forward
    train_menu(chosen_train_number)
  when 7
    $config[:trains][chosen_train_number].move_back
    train_menu(chosen_train_number)
  when 8
...
def route_menu_controller(item, chosen_route_number)
  ...
  when 1
    ...
    route_menu(chosen_route_number)
  when 2
...
def routes_menu_controller(item)
  ...
  case item
  when 1
    create_route ? routes_menu : routes_menu_controller(item)
...
def create_train(item)
  ...
  if type_item == "1"
    ...
    puts "Произошла ошибка ввода, пожалуйста повторите"
    trains_menu_controller(item)
  end
end
...
def change_train_routes(chosen_train_number)
  ...
  if item == 1
    ...
    if (1..$config[:routes].size).include?(route_item)
      ...
      puts "Такого маршрута не существует, вы были возвращены на предыдущий экран"
      change_train_routes(chosen_train_number)
    end
  else
    puts "Такого пункта не существует, повторите ввод"
    change_train_routes(chosen_train_number)
  end
end
...
def create_route
  ...
  if start_station_item == end_station_item
    puts "Конечная станция не может равняться начальной, повторите ввод"
    return false
  else
    if ($config[:stations][start_station_item - 1] != nil) && ($config[:stations][end_station_item - 1] != nil)
      ...
      puts "Произошла ошибка ввода пунктов меню, повторите ввод"
      return false
    end
  end
end
...
def add_station_to_route(chosen_route_number)
  ...
  if (1..$config[:stations].size).include?(station_item)
    ...
    if (all_route_stations.size == 2) && (all_route_stations.include?(target_station))
      puts "Нельзя добавлять начальную и конечную стации как промежуточную, повторите ввод"
      add_station_to_route(chosen_route_number)
    else
      $config[:routes][chosen_route_number].add_station(target_station)
    end
  else
    puts "Вы ввели неверный номер станции, повторите ввод"
    add_station_to_route(chosen_route_number)
  end
end
